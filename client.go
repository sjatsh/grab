...
// responses are sent through the given Re sponse channel as soon as they are
...
func (c *Client) DoBatch(ctx context.Context, opt *DownloadOptions, requests ...*Request) <-chan *Response {
	...
	for i := 0; i < opt.workers; i++ {
		...
		go func() {
			defer wg.Done()
			c.DoChannel(reqch, respch, &respNum)
		}()
	}
...
func (c *Client) validateLocal(resp *Response) stateFunc {
	...
	if resp.CanResume && !resp.DownloadParts {
...
func (c *Client) getRequestParts(resp *Response) stateFunc {
	...
	wg := sync.WaitGroup{}
	wg.Add(resp.Request.DownloadOptions.workers)
	...
	go func() {
		for res := range chResults {
			resLock.Lock()
			downloadingJobs = append(downloadingJobs, res)
			resLock.Unlock()
		}
	}()
...
func (c *Client) copyFile(resp *Response) stateFunc {
	...
	if resp.DownloadParts {
		...
		resp.bytesResumed = 0
		for _, v := range resp.Chunks {
			resp.bytesResumed += v.Completed
		}

		if !resumableFlag {
			// 创建文件
			nfile, err := os.OpenFile(resp.Request.Filename, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0660)
			if err != nil {
				resp.err = err
				return c.closeResponse
			}
		...
		resp.MultiCPInfo = resumableInfo
		return c.getRequestParts
	}
